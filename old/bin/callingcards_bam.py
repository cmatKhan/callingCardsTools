...
# local dependencies
from callingcardstools.bam_parsers import main
...
def main(args=None):
    ...
    (passing_reads_df,failing_reads_df) = count_hops(args.bampath,
                                                     require_exact_length,
                                                     int(args.mapq_filter)

    # create output paths
    out_dict = {
        'passing': os.path.splitext(os.path.basename(args.bampath))[0] + "_passing.bed",
        'failing': os.path.splitext(os.path.basename(args.bampath))[0] + "_failing.bed"
    }
