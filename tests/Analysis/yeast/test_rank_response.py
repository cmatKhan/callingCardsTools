...
def test_read_in_data_columns():
    ...
    df = read_in_data(test_data_path,
                      test_identifier_col,
                      test_effect_col,
                      test_pval_col,
                      test_source,
...
def test_create_partitions():
    ...
    expected = np.concatenate(
        [np.repeat(np.arange(1, 11), 10), np.repeat(11, 3)])
    result = create_partitions(vector_length, equal_parts)
    assert np.array_equal(
        result, expected), "Failed for division with remainder"
...
def test_label_responsive_genes():
    ...
    result_df_normalized = label_responsive_genes(
        df, 0.6, 0.05, 1)

    assert all(result_df_normalized['responsive'] ==
               expected_responsive_normalized), \
...
def test_calculate_rank_response():
    df = pd.DataFrame({
        ...
        'binding_pvalue': [.05, .3, .004, .0002, .00006]
    })
    ...
    # Call the function
    bin_size = 2
    result_df = bin_by_binding_rank(df, bin_size, order_by_effect=True)

    expected_df = expected_df.sort_values(by=['binding_effect',
                                              'binding_pvalue'],
    ...
    pd.testing.assert_frame_equal(
        result_df, expected_df.reset_index(drop=True))

    result_df = bin_by_binding_rank(df, bin_size)

    expected_df = expected_df.sort_values(by=['binding_pvalue',
                                              'binding_effect'],
...
def test_rank_response_ratio_summarize():
    ...
    test_df = pd.DataFrame({
        ...
        'expression_pvalue': [0.5, 0.007, .12],
        'binding_effect': [1.04, 3.07, 2.01],
        'binding_pvalue': [.1, 0.02, 1.0]
    })

    # Call the function
    df, random_expectation_df, rank_response_df = \
        rank_response_ratio_summarize(test_df, bin_size=2)

    # Perform assertions
    assert isinstance(df, pd.DataFrame), \
        "Returned df is not a DataFrame"
    assert isinstance(random_expectation_df, pd.DataFrame), \
