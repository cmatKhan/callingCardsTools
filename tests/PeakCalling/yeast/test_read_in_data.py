...
def test_relabel_chr_column():
    # Setup
    data_df = pd.DataFrame({"chr": ["chr1", "chr2", "chr3"], "start": [1, 2, 3]})
    ...
    # Call function
    relabeled_df = relabel_chr_column(
        data_df, chrmap_df, "curr_chr_name_convention", "new_chr_name_convention"
    )
...
def test_read_in_experiment_data(tmp_path):
    ...
    qbed_data = pd.DataFrame(
        {"chr": ["chr1"], "start": [1], "end": [2], "depth": [1], "strand": ["+"]}
    )
    ...
    experiment_pr, experiment_total_hops = read_in_experiment_data(
        str(qbed_file), "curr_chr_name_convention", "new_chr_name_convention", chrmap_df
    )

    # Assertions
    assert list(experiment_pr.df.columns) == ["Chromosome", "Start", "End", 'depth', "Strand"]
...
def test_read_in_background_data(tmp_path):
    ...
    # Assertions
    assert list(background_pr.df.columns) == ["Chromosome", "Start", "End", "depth", "Strand"]
