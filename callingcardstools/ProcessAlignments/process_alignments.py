...
def process_alignments(args: argparse.Namespace) -> None:
    ...
    logging.info(
        f'adding batch {barcode_details.barcode_dict["batch"]} data...')
    db.add_batch_qc(
        barcode_details,
        args.id_to_barcode_map)
    ...
    # TODO CRITICAL replace is set so that when there are replicates,
    # the repeated calls to the same TF update the table
    db.peak_caller(
        if_exists='append',
