...
def poisson_pval_factory(
    ...
    def pval(bg_hops, expr_hops):
        ...
        results_dict = {'pval': 1-scistat.poisson.cdf(x, mu),
...
def add_pvalues(
    poisson_pval: Callable[[int, int], float],
    hypergeom_pval: Callable[[int, int], float]) -> Callable[[pd.Series], float]:  # noqa
    """Calculate any number of pvalues for a row of a given dataframe

    Args:
        poisson_pval (Callable[[int,int],float]): a poisson pvalue
         calculator function
        hypergeom_pval (Callable[[int,int],float]): a hypergeometric pvalue
         calculator function

    Returns:
        Callable[[pd.Series],float]: A function which returns
         any number of pvalues
    """
...
def call_peaks_with_background(qbed_df: pd.DataFrame,
    ...
    try:
        if not set(grouped_df.groups) - set(total_hops_dict) == set():
            raise KeyError(
                "All group names must be keys in the total_hops_dict")
    except AttributeError as err:
        raise AttributeError(
            "The dataframe must be grouped, even if only a group of 1") from err # noqa
